...
def main():
    if not IN_ZIP.exists():
        raise FileNotFoundError(f"Cannot find {IN_ZIP}. Upload the TSV zip there.")
    ...
    if COL_KI not in df.columns:
        raise RuntimeError(f"Column '{COL_KI}' not found. Check you picked a *_tsv.zip file.")
    ...
    def drug_id(row):
        if pd.notna(row.get(COL_PUBCHEM, np.nan)):
            return f"CID{int(row[COL_PUBCHEM])}"
        if pd.notna(row.get(COL_MONOMER, np.nan)):
            return f"BDB{int(row[COL_MONOMER])}"
    ...
    # We don't have cellular contexts here; define one pseudo-context “BDB” with 32D vector
    # from PCA over drug space mean (simple, reproducible)
    ...
    ce["disease"] = 1  # mark as “disease context” for the MVP
    ce.to_csv(OUT/"cell_embeddings.csv", index=False)

    # ---- drug_catalog.csv ----
    # Pull names from any available source in the original DF for kept drugs
    ...
    drugs["approved"] = 0     # BindingDB doesn’t carry this cleanly; fill 0 (can enrich later)
    drugs["clinical_phase"] = 0
    drugs.to_csv(OUT/"drug_catalog.csv", index=False)

    # ---- train_triples.csv ----
    # Define positives/negatives by potency thresholds on Ki.
    # Positive if Ki <= 100 nM (strong binders), negative if Ki >= 10000 nM.
    ...
    def mk_rows(frame, label):
        # de_logfc proxy = pKi minus per-gene median pKi (centered potency)
